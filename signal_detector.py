...
class SignalClassifier(tf.keras.Model):
    def __init__(self, nblocks, ksize, filters):
        ...
        # self.conv_0 = tf.keras.layers.Conv1D(filters, ksize, activation='relu')
        self.conv_final = tf.keras.layers.Conv1D(1, 1, activation='linear')
        self.sigmoid_final = tf.keras.layers.Activation('sigmoid')
