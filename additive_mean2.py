...
eta_0 = tf.Variable(tf.constant(0.0))
log_r = tf.Variable(tf.constant(0.0))
var_list = [eta_0, eta_t, log_r]
...
def get_loglikelihood():
    #
    r = tf.math.exp(log_r)
    mu = tf.math.exp(eta_0) + tf.math.exp(eta_t)
    p = r / (r + mu)
    #
    logp_negbinom =\
...
def get_loss(return_all=False):
    mu_t = tf.math.exp(eta_t)
    diffs = D @ tf.expand_dims(mu_t, 1)
    tv = 0.1 * tf.reduce_sum(tf.math.abs(diffs))
    reg = 0.1 * tf.reduce_sum(tf.math.abs((mu_t)))
...
# initialize logodds
eta_t.assign(tf.math.log(1e-9 + obs))
