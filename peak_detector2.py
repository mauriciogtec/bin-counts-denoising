...
class Classifier(tf.keras.Model):
    def __init__(self, nblocks, ksize, filters, head_nblocks):
        super(Classifier, self).__init__()
        self.nblocks = nblocks
        self.head_nblocks = head_nblocks
        self.ksize = ksize
        self.filters = filters
        self.blocks = []
        ...
        for i in range(nblocks):
            ...
            self.blocks.append(new_block)
        self.blocks_signal = []
        for i in range(self.head_nblocks):
            new_block = ResBlock(ksize, filters)
            self.blocks_signal.append(new_block)
        self.blocks_peaks = []
        for i in range(self.head_nblocks):
            new_block = ResBlock(ksize, filters)
            self.blocks_peaks.append(new_block)
